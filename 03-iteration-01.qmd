...
In this session we will cover another way to reduce code duplication:
iteration.
...
-   which columns you want to iterate over:
...
-   we could use colon notation, `bill_length_mm:body_mass_g`, because
...
-   `.cols` uses same specification as `select()`: `starts_with()`,
...
-   all of `bill_length_mm`, `bill_depth_mm`, `flipper_length_mm`,
    `body_mass_g`, `year`
...
-   we can pass a function, `sd_error` to `across()` since R is a
...
[^1]: There is no problem when we use `sd_error()` because we accounted
    for NA in our function definition
...
The solution is to create a new function that calls `mean()` with
`na.rm = TRUE`
...
It makes sense to use `across()` to apply the transformation to all
three variables
...
🎬 Write a function that summarises multiple specified columns of a data
frame
