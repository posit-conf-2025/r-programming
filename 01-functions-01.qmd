...
1.  vector functions: one of more vectors as input, one vector as output
    ...
    i.  output same length as input. "mutate" functions will work well
        in `mutate()` and `filter()`. Principles of writing functions
...
proportion trimmed from each end \[\^1\]
...
🎬 Write a function that performs the Box-Cox power transformation using
...
This is known as data-masking: the dataframe environment masks the user
environment by giving priority to the dataframe.
...
🎬 Write a function to calculate the median, maximum and minimum values
