...
author: "Emma Rand and Ian Lyttle <br>WiFi: On your badges"
format: 
  revealjs:
...
🦋[\@3mmarand.bsky.social](https://bsky.app/profile/3mmarand.bsky.social)
...
🦋[\@ijlyttle.bsky.social](https://bsky.app/profile/ijlyttle.bsky.social)
...
:::: {.column width="32%"}
::: {style="font-size: 40%;"}
![](images/yorkshire.png){width="300"}

*Ingredients*: Sugar, glucose syrup, acid: citric acid; flavouring,
colours: E102, E110, E122, E124, E133, E142.
:::
::::

:::: {.column width="32%"}
::: {style="font-size: 40%;"}
![](images/ponte.png){width="250"}

*Ingredients*: Treacle, glucose syrup, maize starch, invert sugar syrup,
wheat flour (wheat flour, calcium carbonate, iron, niacin, thiamin),
liquorice extract, modified potato starch, vegetable oils (palm,
sunflower, coconut), flavouring, glazing agent (carnauba wax).
:::
::::
:::::::::
...
🎬 Detailed instructions for installing these were covered in
...
🔑️ You may think you have to write complex functions - you don't! Start
...
The Box-Cox transformation is a statistical technique that converts
non-normally distributed data to an approximately normal distribution by
applying a power-transformation based on a parameter lambda ($\lambda$).
...
🎬 Your task is to write a function that performs the Box-Cox power
transformation
...
![Distribution of values before (left) and after (right) a Box-Cox
transformation.](images/box-cox.png)

## Your turn: Box-Cox

1.  Set the default $\lambda = 1$

2.  Still have time? In fact, the transformation only applies when
    $\lambda \ne 0$. Check if $\lambda = 0$ and if it does, set it to
    0.0001 and print "Lambda cannot be 0, setting to 0.0001"

3.  Still have time? In fact, the transformation is defined as $log(x)$
    when $\lambda = 0$. Amend the function so it performs the
    appropriate transformation depending on the value of $\lambda$.
...
minimum values of a variable. Incorporate an argument to allow the
