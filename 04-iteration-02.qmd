...
## Map {auto-animate=true, auto-animate-unmatched=false}
...
## Filter (Keep) {auto-animate=true, auto-animate-unmatched=false}
...
## Filter (Keep) {auto-animate-unmatched=false}
...
## Reduce {auto-animate=true}

```r
num <- 1:4
reduce(num, \(acc, x) acc + x)
```
...
## Reduce {auto-animate=true, auto-animate-unmatched=false}
...
- sometimes easier just to say `sum`, which will take two arguments
- `.init`
